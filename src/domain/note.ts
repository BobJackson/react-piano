...
export const NATURE_PITCH_INDICES: PitchIndex[] = [
    0,
    2,
    4,
    5,
...
export function fromMidi(midi: MidiValue): Note {
    ...
    const octave = (Math.floor(pianoRange / SEMITONE_IN_OCTAVE) + 1) as OctaveIndex

    const index = pianoRange % SEMITONE_IN_OCTAVE
    const pitch = PITCH_REGISTRY[index]

    const isSharp = !NATURE_PITCH_INDICES.includes(index)
